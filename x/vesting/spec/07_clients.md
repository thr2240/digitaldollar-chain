...
Find below a list of `digitaldollard` commands added with the `x/vesting` module. You can obtain the full list by using the `digitaldollard -h` command.
...
```go
digitaldollard add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
digitaldollard query vesting balances ADDRESS [flags]
```
...
```go
digitaldollard tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
digitaldollard tx vesting clawback ADDRESS [flags]
```
...
```go
digitaldollard tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
