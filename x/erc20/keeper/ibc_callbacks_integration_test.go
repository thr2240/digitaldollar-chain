...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.Adigitaldollardenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    claimstypes.DefaultClaimsDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.Adigitaldollardenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert karma", func() {
			// Register 'karma' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'karma' thru IBC
			...
			// 1. Send karma from Evmos to Osmosis
			...
			// check ibc karma coins balance on Osmosis
			...
			// 2. Send karma IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.Adigitaldollardenomtrace.Path, teststypes.Adigitaldollardenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.poseIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc karma coins balance on Osmosis - should be zero
			...
			// check karma balance after transfer - should be equal to initial balance
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			// should trigger claims logic and send karma coins from claims to receiver
