...
Find below a list of  `digitaldollard` commands added with the  `x/erc20` module. You can obtain the full list by using the `digitaldollard -h` command. A CLI command can look like this:

```bash
digitaldollard query erc20 params
```
...
```bash
digitaldollard tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
digitaldollard tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
digitaldollard tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
