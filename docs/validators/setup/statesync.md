...
```bash
digitaldollard init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .digitaldollard/config folder

```bash
mv $HOME/genesis.json ~/.digitaldollard/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.digitaldollard/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.digitaldollard/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.digitaldollard/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.digitaldollard/config/config.toml
```

### Create digitaldollard service

```bash
echo "[Unit]
Description=digitaldollard Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which digitaldollard) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/digitaldollard.service; sudo mv $HOME/digitaldollard.service /etc/systemd/system/
```

```bash
sudo systemctl enable digitaldollard.service && sudo systemctl daemon-reload
```

### Run digitaldollard

```bash
sytemctl start digitaldollard
```

### Check logs

```bash
journalctl -u digitaldollard -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.digitaldollard/config/config.toml
```
