...
echo "export DAEMON_NAME=digitaldollard" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.digitaldollard" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.digitaldollard`) and copy over the current binary.

```bash
mkdir -p ~/.digitaldollard/cosmovisor
mkdir -p ~/.digitaldollard/cosmovisor/genesis
mkdir -p ~/.digitaldollard/cosmovisor/genesis/bin
mkdir -p ~/.digitaldollard/cosmovisor/upgrades

cp $GOPATH/bin/digitaldollard ~/.digitaldollard/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `digitaldollard` are the same:
...
```bash
sudo tee /etc/systemd/system/digitaldollard.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.digitaldollard"
Environment="DAEMON_NAME=digitaldollard"
...
sudo -S systemctl enable digitaldollard
sudo -S systemctl start digitaldollard
```

You can check the status with:

```bash
systemctl status digitaldollard
```
