...
Before upgrading the Evmos version. Stop your instance of `digitaldollard` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `digitaldollard` is running.
:::
...
```bash
rm $HOME/.digitaldollard/config/addrbook.json
