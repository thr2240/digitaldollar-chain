...
- Edit `addr` to point to your `digitaldollard` instance.
- Adjust `chain-id` to match your `.digitaldollard/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `digitaldollard`.
...
The KMS may complain that it cannot connect to `digitaldollard`. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.digitaldollard/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `digitaldollard`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell digitaldollard the key that we've got in the previous section.

```bash
digitaldollard gentx --name kmsval --pubkey <pub_key>
```

Now start `digitaldollard`. You should see that the KMS connects and receives a signature request.
