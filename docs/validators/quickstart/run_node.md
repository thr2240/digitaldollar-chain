...
- [`digitaldollard`](./binary.md) {prereq}
...
```bash
digitaldollard start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `digitaldollard keys add $KEY` in `./local_node.sh` with:

```bash
echo "your mnemonic here" | digitaldollard keys add $KEY --recover
```
...
```bash
digitaldollard config keyring-backend file
```
...
```bash
digitaldollard start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001pose --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
::: danger
If you are running a **validator node**, always be careful when doing `digitaldollard unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
...
```shell
rate(digitaldollard_transactions_processed[1m])
```
