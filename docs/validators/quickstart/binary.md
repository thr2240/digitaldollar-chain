...
`digitaldollard` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
### Using `digitaldollard`

After you have obtained the latest `digitaldollard` binary, run:

```bash
digitaldollard [command]
```
...
::: tip
You can also enable auto-completion with the `digitaldollard completion` command. For example, at the start of a bash session, run `. <(digitaldollard completion)`, and all `digitaldollard` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.digitaldollard` directory.
...
To specify the `digitaldollard` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.digitaldollard/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.digitaldollard/config/app.toml` file.
...
We can view the default client config setting by using `digitaldollard config` command:

```bash
digitaldollard config
{
 "chain-id": "",
 "keyring-backend": "os",
 "output": "text",
...
A list of commonly used flags of `digitaldollard` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.digitaldollard`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `digitaldollard` commands. You can obtain the full list by using the `digitaldollard -h` command.
