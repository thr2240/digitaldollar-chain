...
```bash
digitaldollard keys [command] --help
```
...
MY_VALIDATOR_ADDRESS=$(digitaldollard keys show dev0 -a)
```
...
yes $KEYPASSWD | digitaldollard keys add me
yes $KEYPASSWD | digitaldollard keys show me
# start digitaldollard with keyring-backend flag
digitaldollard --keyring-backend=file start
```
