...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`digitaldollard`](../../validators/quickstart/binary.md), use the following command with `register-coin`:

```bash
digitaldollard tx gov submit-proposal register-coin <path/to/metadata.json> \
  --title=<title> \
...
```bash
digitaldollard tx gov submit-proposal register-coin --proposal=<path/to/proposal.json>
```
