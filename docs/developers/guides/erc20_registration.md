...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`digitaldollard`](../../validators/quickstart/binary.md), use the following command with `register-erc20`:

```bash
digitaldollard tx gov submit-proposal register-erc20 ERC20_ADDRESS...\
  --title=<title> \
...
```bash
digitaldollard tx gov submit-proposal register-erc20 --proposal=<path/to/proposal.json>
```
